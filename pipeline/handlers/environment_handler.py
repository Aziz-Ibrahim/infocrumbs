...
def handle_environment_articles(articles):
    ...
    :param articles: List of dictionaries containing article data from
    NewsData.io.
    ...
    for article in articles:
        ...
        if not title or not url:
            print(
                f"Skipping environment article due to missing title or "
                f"URL: {article}"
            )
            continue
        ...
        try:
            ...
            if matched_topic_for_tag and \
                matched_topic_for_tag != environment_topic:
                crumb.tags.add(matched_topic_for_tag.name)

        except Exception as e:
            print(
                f"Error saving environment article (Title: {title[:50]}...): "
                f"{e}"
            )
            continue
